...
middleware, or by any in-process caller that wants one) and read by the
structured log formatter (``app/logging_config.py``). Everything downstream —
including work spawned by ``asyncio.create_task`` from inside the request,
since contextvars propagate through asyncio task creation — picks it up with
no parameter threading required.

Deliberately framework-free (contextvars is stdlib) and living in
``features/`` rather than ``app/``: correlation-ID propagation is a portable
core concern, not an HTTP-only one.
...
_request_id_var: contextvars.ContextVar[str] = contextvars.ContextVar(
    "auth_service_request_id", default=""
)
